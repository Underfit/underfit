...
def chi2_test_statistic(M, Obs, K, num_M, num_Obs):
    ...
    pv = theano.function([],p)
    mv = theano.function([],M)
    pZEROs = theano.shared(np.asarray(pv() == 0.0, dtype = theano.config.floatX))
    mZEROs = theano.shared(np.asarray(mv() == 0.0, dtype = theano.config.floatX))
